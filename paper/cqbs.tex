...
%\begin{figure*}[!ht]
%\centering
%\includegraphics[width=.8\linewidth]{figs/evalutequery}
%\caption{The basic flow of operations for evaluating a query for syndication. Metadata keys are extracted
%from the parsed query and are inserted into several broker data structures that allow the broker to efficiently
%find queries needing reevaluation.}
%\label{fig:evaluatequery}
%\end{figure*}
%
...
\todo{add back in the evaluatequery figure. Change the final "select" step into the set of forwarding UUIDs}
...
\todo{change the figure message so it actually has a building value on the initial publisher}
